...
class Channel(YoutubeBase):
    ...
    def __init__(self, channel_id: str) -> None:
        ...
        self.dict_to_load = (self.get_service().channels().list(
            id=self.__channel_id, part='snippet,statistics').execute())
        self.title = self.dict_to_load["items"][0]["snippet"]["title"]
        self.description = (
            self.dict_to_load)["items"][0]["snippet"]["description"]
        self.url = (
            self.dict_to_load)[
            "items"][0]["snippet"]["thumbnails"]["default"]["url"]
        self.subscriber_count = (
            self.dict_to_load)["items"][0]["statistics"]["subscriberCount"]
        self.video_count = (
            self.dict_to_load)["items"][0]["statistics"]["videoCount"]
        self.viewing_count = (
            self.dict_to_load)["items"][0]["statistics"]["viewCount"]
